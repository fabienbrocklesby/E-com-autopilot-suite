...
import { AppError, Thread, ThreadListItem, ThreadDetail, Draft, Message } from "../types/index.ts";
...
threadsRouter.get("/", async (c) => {
  ...
  const rows = await query<ThreadListItem>(
    `SELECT
     ...
     LIMIT $2 OFFSET $3`,
    [statusFilter ?? null, limit, offset],
  );
...
threadsRouter.patch("/:id/drafts/:draftId", async (c) => {
  ...
  if (affected === 0) throw new AppError(404, "Draft not found");

  // When a draft is approved, send the reply via Gmail immediately.
  if (body.status === "approved") {
    const [draft, thread, lastInbound] = await Promise.all([
      queryOne<Draft>("SELECT * FROM drafts WHERE id = $1", [draftId]),
    ...
    if (draft && thread && lastInbound) {
      // Get the connected email account.
      const tokenRow = await queryOne<{ email: string }>(
        "SELECT email FROM oauth_tokens ORDER BY id DESC LIMIT 1",
      );
      ...
      // Mark the draft as sent and the thread as replied.
      await Promise.all([
        execute("UPDATE drafts SET status = 'sent' WHERE id = $1", [draftId]),
        execute("UPDATE threads SET status = 'replied' WHERE id = $1", [threadId]),
      ]);
    }
  }
