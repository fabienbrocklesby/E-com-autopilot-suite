...
import { setupGmailWatch, processNewMessages } from "./gmail.ts";
...
export function startWatchRenewalLoop(): ReturnType<typeof setInterval> {
  ...
  return setInterval(() => {
    renewExpiringWatches().catch((err) =>
      console.error("[watch] Renewal check failed:", err)
    );
...
export function startFallbackPoller(): ReturnType<typeof setInterval> {
  return setInterval(() => {
    pollAllAccounts().catch((err) =>
      console.error("[watch] Fallback poll failed:", err)
    );
  }, FALLBACK_POLL_INTERVAL_MS);
}

async function renewExpiringWatches(): Promise<void> {
  const tokens = await query<Pick<OAuthToken, "email" | "workspace_id">>(
    "SELECT email, workspace_id FROM oauth_tokens",
  );
...
async function pollAllAccounts(): Promise<void> {
  const tokens = await query<Pick<OAuthToken, "email" | "last_history_id">>(
    "SELECT email, last_history_id FROM oauth_tokens WHERE last_history_id IS NOT NULL",
  );
